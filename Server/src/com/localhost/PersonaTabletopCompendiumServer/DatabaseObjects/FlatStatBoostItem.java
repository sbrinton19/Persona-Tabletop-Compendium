...
public class FlatStatBoostItem extends FlatItem {
	private static String STATBOOSTITEMSEARCH = null;
	private static String STATBOOSTITEMINSERT = null;
	private static String STATBOOSTITEMUPDATE = null;
	...
	private void initSUIDStrings() {
		if (FlatStatBoostItem.STATBOOSTITEMSEARCH != null)
			return;
		FlatStatBoostItem.STATBOOSTITEMSEARCH = "SELECT * FROM item WHERE item.itemid = ? AND item.type = ?";
		...
		FlatStatBoostItem.STATBOOSTITEMINSERT = String.format(insertTemplate, built[0], built[1]);
		FlatStatBoostItem.STATBOOSTITEMUPDATE = String.format(updateTemplate, built[2]);
	}
	...
	protected boolean isIgnoredField(String name) {
		return name.equals("STATBOOSTITEMINSERT") || name.equals("STATBOOSTITEMUPDATE") || name.equals("STATBOOSTITEMSEARCH")
				|| name.equals("STATBOOSTITEMDELETE");
	}
	 ...
	 * @throws SQLException
	 */
	public ResultSet databaseSelectStatBoostItem(Connection conn) throws SQLException {
		PreparedStatement search = conn.prepareStatement(FlatStatBoostItem.STATBOOSTITEMSEARCH);
	...
	private boolean updateOrInsert(Connection conn) {
		...
		try {
			ResultSet rs = this.databaseSelectStatBoostItem(conn);
			boolean isInsert;
			if (!rs.isBeforeFirst()) {
				// No data so blind insert
				state = conn.prepareStatement(FlatStatBoostItem.STATBOOSTITEMINSERT);
				isInsert = true;
			} else {
				state = conn.prepareStatement(FlatStatBoostItem.STATBOOSTITEMUPDATE);
				isInsert = false;
			}
	 ...
	 * @param conn
	 */
	public void databaseDeleteStatBoostItem(Connection conn) {
