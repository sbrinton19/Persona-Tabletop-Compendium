...
CREATE TABLE `shadow` (
  ...
  `maxDamageDice` tinyint(1) unsigned NOT NULL,
  `damageDie` tinyint(1) unsigned NOT NULL,
  `note` varchar(100) DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `id_UNIQUE` (`id`),
  KEY `personaFK_idx` (`personaId`),
  CONSTRAINT `personaFK` FOREIGN KEY (`personaId`) REFERENCES `persona` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `shadow_AFTER_DELETE` AFTER DELETE ON `shadow` FOR EACH ROW BEGIN
